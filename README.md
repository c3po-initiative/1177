...
|---|---|---|
| Personnummer used for login | `Patient/me` and `Patient/{personnummer}` | Synthesized (identifier only) |
...
| `POST /journalcategories/journaloverview/{polltimeline,detailview}` | `DocumentReference` (default; the full timeline) and `Encounter` (rows with `data-cy-journal-overview-item-type="CareContact"`) | DocumentReference: skeleton. Encounter: skeleton (deep parse pending — see `MAPPING-GAPS.md`). |
| `POST /journalcategories/referralStatus/poll` | `ServiceRequest` | Skeleton |
...
| `POST /journalcategories/attentionSignals/poll` | `AllergyIntolerance` | Skeleton (aria-label deep parse pending) |
...
| `POST /LogsAndShare/JournalLog/PollUserAccessLogs` | `AuditEvent` (default — patient self-access) | Skeleton |
| `POST /LogsAndShare/JournalLog/PollJournalLogs` | `AuditEvent?agent=clinician` | Skeleton |
